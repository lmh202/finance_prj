...
export default function Page() {
  ...
  /* ---------- client identity + hydration ---------- */
  useEffect(() => {
    let id = localStorage.getItem("prism_client");
    if (!id || !/^[A-Za-z0-9-]{6,64}$/.test(id)) {
      id = crypto.randomUUID();
      localStorage.setItem("prism_client", id);
    }
    setClientId(id);
  }, []);
  ...
  useEffect(() => {
    if (!clientId) return;
    fetch(`/api/portfolio?client=${clientId}`)
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d.holdings) && d.holdings.length > 0) {
          setHoldings(d.holdings);
          if (d.mode === "shares") setMode("shares");
          lastSaved.current = JSON.stringify({ h: d.holdings, m: d.mode ?? "weight" });
        }
        setHydrated(true);
      })
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      fetch("/api/portfolio", {
        ...
        body: JSON.stringify({ client: clientId, mode, holdings }),
      }).catch(() => {});
    }, 700);
    return () => clearTimeout(t);
  }, [holdingsKey, mode, hydrated, clientId, holdings]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const json = (await res.json()) as { ok: boolean; error?: string } & Partial<AnalyzeResponse>;
        if (ctrl.signal.aborted) return;
        if (json.ok && json.dates) {
          setResult(json as AnalyzeResponse);
        } else {
          setError(json.error || "Analysis failed for this combination.");
        }
      } catch {
        if (!ctrl.signal.aborted) setError("Network error — could not reach the analytics engine.");
      } finally {
