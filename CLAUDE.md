...
This repo enforces a strict ownership model — violating it breaks other
developers' branches.
...
| `DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL` / `DEEPSEEK_MODEL` | you (Dev 4) | Explanation-only LLM text in `recommendation/llm_client.py`, used on the fallback decision path. Missing key → deterministic template. See `.env.example`. |
...
backend/                     FastAPI service
  ...
  routers/                   8 routers: one per owned engine (health, strategy, news,
  ...
  src/
    ...
    recommendation/          Developer 4 — the production decision + its explanation layer
    risk_engine/             solo, added later — risk_estimate(s), portfolio_risk (HAR-X + News)
...
|---|---|---|---|
| `portfolio_health/` | Dev 1 | `compute_health()`, `what_if_health()` | Quant formulas + validation study |
| `daily_strategy/` | Dev 2 | `classify_regime()`, `score_assets()`, `backtest()` | Rule-based (production) + scikit-learn walk-forward ML candidate |
| `news_intelligence/` | Dev 3 | `fetch_headlines()`, `essential_news()`, `sentiment_features()` | Keyword rules + FinBERT (live), FinBERT/VADER (batch) |
| `recommendation/` | Dev 4 | `reaction_risk()`, `recommend_daily()`, `recommend_event()`, `apply_constraints()` | Constrained optimisation + explanation layer |
| `risk_engine/` | solo | `risk_estimate()`, `risk_estimates()`, `portfolio_risk()`, `model_available()` | HAR-X volatility + Filtered Historical Simulation |
...
artifacts and candidates; Dev 3's historical sentiment feature table is cached
